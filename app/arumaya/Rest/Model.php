...
abstract class Model implements ArrayAccess, Arrayable, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    public function __construct($attributes = [])
    {
        ...
        $this->connection = 'arumaya.core.sea.';
        $this->timeout = 120;
        $this->headers = [
        ...
        $this->client = new Client([
            'base_url' => 'http://'.\Config::get($this->connection.'address').':'.\Config::get($this->connection.'port'),
            'defaults' => [
                'headers' => [
                    'Accept' => 'application/json',
                    'Content-Type' => 'application/json'
    ...
    public function post(string $uri, array $options = []){
        $base_url = 'http://'.\Config::get($this->connection.'address').':'.\Config::get($this->connection.'port');
        try {
            $result = $this->client->request('POST', $base_url.$uri, [
        ...
        /*$request = $this->client->createRequest('GET', 'http://httpbin.org', [
            'headers' => ['X-Foo' => 'Bar']
        ]);
     ...
     ['json' => [
        ...
        //$res = $this->client->post(['config' => ['URI' => 'bar']], $this->connection, $this->attributes);
    ...
    public function get(string $uri, array $options = []){
        $base_url = 'http://'.\Config::get($this->connection.'address').':'.\Config::get($this->connection.'port');
        try {
            $result = $this->client->request('GET', $base_url.$uri, ['query' => $options]);
            $output = json_decode($result->getBody());
        }catch (RequestException $e) {
            dd($e->getMessage());
            $output = $e->getMessage();
    ...
    public function patch(string $uri, array $data = [], array $query = []){
        $base_url = 'http://'.\Config::get($this->connection.'address').':'.\Config::get($this->connection.'port');
        try {
            $result = $this->client->request('PATCH', $base_url.$uri, [
                'timeout' => $this->timeout,
                'json' => $data,
                'query' => $query,
                'headers' => $this->headers,
    ...
    protected function asDateTime($value)
    {
        ...
        if (preg_match('/^(\d{4})-(\w{3})-(\d{1,2})$/', $value)) {
            return Carbon::createFromFormat('Y-f-d', $value)->startOfDay();
        }
        if (preg_match('/^(\d{1,2})-(\w{3})-(\d{4})$/', $value)) {
            return Carbon::createFromFormat('d-F-Y', $value)->startOfDay();
        }
